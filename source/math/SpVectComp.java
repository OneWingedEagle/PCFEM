...
public class SpVectComp {
	...
	public SpVectComp(SpVect sv, int L){
	...
	public SpVectComp addGeneral(SpVectComp vc){
	
		if(length!=vc.length) throw new IllegalArgumentException("Vectrs have different lengths.");

	 VectComp sum=new VectComp(length);

			for(int i=0;i<nzLength;i++){
				sum.el[this.index[i]]=this.el[i].deepCopy();
			}
			
			for(int i=0;i<vc.nzLength;i++){
				sum.el[vc.index[i]]=sum.el[vc.index[i]].add(vc.el[i]);
			}
			//v.shownz();
	//	v.vectForm().show();
		SpVectComp sumComp= new SpVectComp(sum.el);
